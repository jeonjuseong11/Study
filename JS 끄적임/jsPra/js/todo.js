// 웹 요소 가져오기
const todoInput1 = document.querySelector("#todo-input1"); // 사용자 입력
const todoInput2 = document.querySelector("#todo-input2"); // 사용자 입력
...
function addTodo(e) {
  ...
  console.log(todoInput2.value);
  const inputDate = new Date(todoInput2.value);
  const remainTime = inputDate.getTime() - today.getTime();
  const remainDay = Math.round(remainTime / (24 * 60 * 60 * 1000)); //남은 날짜 계산

  const newDiv = document.createElement("div");
  newDiv.classList.add("todo");
  const newTodo = document.createElement("li");
  newTodo.innerText = `할일 : ${todoInput1.value}  마감일 : ${todoInput2.value} 남은시간 : ${remainDay}일 `;
  newTodo.classList.add("todo-content");
  newDiv.appendChild(newTodo); // 새로운 내용 추가

  saveToLocal(todoInput1.value); // 로컬 스토리지에 저장
  saveToLocal(todoInput2.value);
